...
arma::vec bethelRcppOpen(DataFrame &stratif, 
  ...
  arma::dmat a1(s.n_rows,s.n_cols);
  arma::vec cvvec =cv.t();
  arma::vec Nvec= N.t();
  ...
  arma::dvec alfa1=repRcppdouble(oneovernvar, nvar1);
  arma::dvec alfanext1  = repRcpp2(0, nvar1);
  arma::dvec x1= repRcppdouble(0.1, nstrat);
  ...
  arma::dvec n = chromyNew1(alfatot1,diff1,iter, alfa1, alfanext1,x1,a1,cost,epsilon,
  ...
  while (contx > 0 && iter1 < maxiter1) {
    ...
    iter= 0;
    oneovernvar= 1/nvar2;
    alfa1=repRcppdouble(oneovernvar, nvar1);
    alfanext1  = repRcpp2(0, nvar1);
    x1= repRcppdouble(0.1, nstrat);
    ...
    for (int i = 0; i < N.size(); i++) {
      n1(i) = n(i);
      if (n(i) > Nvec(i)) n1(i) = Nvec(i);
      if (n(i) < minnumstrat) {
        if (Nvec(i) >= minnumstrat) n1(i) = minnumstrat;
        if (Nvec(i) < minnumstrat) n1(i) = Nvec(i);
      }
    }
  ...
  for(int i = 0;i <  N.size(); ++i){
    ...
    if (minnumstrat < Nvec[i]) {
      if (outvec[i] < minnumstrat) outvec[i] = minnumstrat;
    }
  }
