...
strataGenalg <- function(errors, strata, cens, strcens, 
  ...
  # }
    ...
    if (strcens == TRUE) {
        ...
        censiti <- 1
        cens <- aggrStrata_RcppOpen(cens, nvar, vett, censiti, dominio)
        dimcens <- nrow(cens)
    }
    #
    # --------------------------------------------------------------------------
    ...
    evaluate <- function(indices) {
        ...
        dimens <- NULL
        censiti <- 0
        strcor <- aggrStrata_RcppOpen(strata, nvar, floor(indices), censiti, 
            dominio)
        dimsamp <- nrow(strcor)
        # change ------------------------------------
        if (strcens == TRUE)
        ...
        # initialStrata)
        soluz <- bethelRcppOpen(strcor, errors, minnumstr, 
...
#     if (writeFiles == TRUE) dev.off()
    ...
    censiti <- 0
    strcor <- aggrStrata_RcppOpen(strata, nvar, v, censiti, dominio)
...
# Here the change to take into account the censused strata
    if (strcens == TRUE) {
      ...
      allstrata <- bethelRcppOpen(stratatot,errors,minnumstr,
                          realAllocation = realAllocation)
